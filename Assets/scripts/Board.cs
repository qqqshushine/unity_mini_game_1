...
public class Board : MonoBehaviour {
   ...
   private void Update () {
      if (canPlay && Input.GetMouseButtonUp (0)) {
         ...
         if (hit)// 真人点击
            HitBox (hit.GetComponent <Box> (), false) ;
      }
      RobotsClick();
   }

   private void RobotsClick () {
      // 简单人机模拟
      if (!isRobots) {
         return;
      }
      ...
      while(true){
         ...
         if((marks[n] == Mark.None)) {
            GameObject childGameObject = transform.GetChild (n).gameObject ;
            HitBox(childGameObject.GetComponent<Box>(), true);
            break;
         }
      }
   }

   private void HitBox (Box box, bool flag) {
      if (!box.isMarked) {
